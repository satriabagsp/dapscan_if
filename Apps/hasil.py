...
def app(selected, pilihNama, conn):
    ...
    if len(df_hasil) == 0:
        ...
        st.write(f'1. Responsif: {rerata_p1}')
        st.write('')
        st.write(f'2. Kualitas: {rerata_p2}')
        st.write('')
        st.write(f'3. Integritas: {rerata_p3}')
        st.write('')
        st.write(f'4. Dapat Diandalkan: {rerata_p4}')
        st.write('')
        st.write(f'5. Transparan : {rerata_p5}')
        # for isi in list_p5:
        #     st.write(f'- {isi}')
        # st.write('')
